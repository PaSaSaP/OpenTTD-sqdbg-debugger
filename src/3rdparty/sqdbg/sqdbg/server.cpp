...
#define SQDBG_SV_VER 9

#include "../include/sqdbg.h"
...
void sqdbg_sleep( int ms )
{
	...
	void _OutputDebugStringFmt( std::string_view sv )
	{
		_OutputDebugString( sv.data() );
	}
#else
...
#undef _SC
	...
	#define _SC(s) s
		...
		typedef uint16_t SQUnsignedChar;
			...
			#define WCHAR_SIZE 4
		#endif
		typedef unsigned int SQUnsignedChar;
	#endif

	STATIC_ASSERT( sizeof(SQChar) == WCHAR_SIZE );
#else
	typedef unsigned char SQUnsignedChar;
#endif

STATIC_ASSERT( sizeof(char) == sizeof(SQUnsignedChar) );

#ifdef SQUSEDOUBLE
typedef uint64_t SQFloat_Int;
#else
typedef unsigned int SQFloat_Int;
#endif

STATIC_ASSERT( sizeof(SQFloat) == sizeof(SQFloat_Int) );
...
#ifndef sq_rsl
#define sq_rsl(l) ((l)*sizeof(char))
#endif

//#define SQStringFromSQChar(_pch) \
//	( (SQString*)( (char*)(_pch) - (char*)offsetof( SQString, _val ) ) )

// assume that _val is last element in SQString and aligned to pointer size bytes
#define SQStringFromSQChar(_pch) \
	( (SQString*)( (char*)(_pch) - (char*)(sizeof SQString) + (char*)(sizeof(char*)) ) )
...
#error "SQUIRREL_VERSION_NUMBER is undefined"
	...
	#define _userdatavalptr(ud) sq_aligning( (ud) + 1 )
	...
	#define _userdatavalptr(ud) (ud->_val)
	typedef SQFUNCTION _SQDEBUGHOOK;
...
#if SQUIRREL_VERSION_NUMBER < 310 && defined(NO_GARBAGE_COLLECTOR)
	#define SS_NOGC(...) __VA_ARGS__
#else
	#define SS_NOGC(...)
#endif
...
#if defined(SUPPORTS_DEREF_OP) && SQUIRREL_VERSION_NUMBER < 300 && defined(SQDBG_NO_RTTI)
	#undef SUPPORTS_DEREF_OP
#endif
...
{
	return SQString::Create( vm->_sharedstate, str.ptr );
}

inline SQString *CreateSQString( SQSharedState *ss, const sqstring_t &str )
{
	return SQString::Create( ss, str.ptr );
}

inline bool SQTable_Get( SS_NOGC(SQSharedState *ss,) SQTable *table, const sqstring_t &key, SQObjectPtr &val )
{
#if SQUIRREL_VERSION_NUMBER >= 310
	#ifdef SQUNICODE
		...
		SQObjectPtr str = SQString::Create( ss, key.ptr, key.len );
	#else
		SQObjectPtr str = SQString::Create( table->_sharedstate, key.ptr );
	#endif
...
{
	// NOTE: Without type and size checking, _rawval equality can return false
	// on 32-bit double builds
	return ( _rawval(o1) == _rawval(o2) && sq_type(o1) == sq_type(o2) );
}

template < int s1size >
inline bool IsEqual( const char (&s1)[s1size], const SQString *s2 )
{
...
public:
	...
	void CallBegin( SQFunctionProto *func )
	{
		...
		{
			char tmp[ FMT_PTR_LEN + 1 ];
			int len = printhex( tmp, FMT_PTR_LEN, (uintptr_t)func );
			tmp[len] = 0;
			tag->funcname = SQString::Create( ss, tmp );
		}

		if ( funcsrc )
		{
			auto v = funcsrc->View();
			unsigned int len = v.length() + 1 + FMT_UINT32_LEN + 1;
			std::span<char> tmp = ss->GetScratchPad( sq_rsl(len) );
			len = v.length();
			...
			{
				tmp[len++] = ':';
				len += printint( tmp.data() + len, FMT_UINT32_LEN, line);
			}

			tmp[len] = 0;
			tag->funcsrc = SQString::Create( ss, tmp.data() );
		}
		else
		{
...
#define PROF_GROUP_NAME_LEN_ALIGNMENT 16
	...
	int GetMaxOutputLen( SQString *tag, int type )
	{
		if ( tag )
		{
			return STRLEN(PROF_GROUP_OUTPUT_TEMPLATE) +
				ROUND( tag->View().length(), PROF_GROUP_NAME_LEN_ALIGNMENT) +
				1;
		}
		...
		switch ( type )
		{
			...
			case 0:
			{
				...
				{
					nodetag_t *node = &m_NodeTags[i];
					len += (int)node->funcsrc->View().length();
					len += (int)node->funcname->View().length();
				}
	...
	// Returns character length
	int Output( SQString *tag, int type, char *buf, int size )
	{
		Assert( size > 0 );
		...
		if ( tag )
		{
			...
			len = group->tag->View().length();
			memcpy( buf, group->tag->View().data(), sq_rsl(len));
			buf += len; size -= len;
...
private:
	...
	void DoPrint( const vector< node_t > &nodes, hnode_t i,
			sample_t totalSamples, int depth, char *&buf, int &size )
	{
		const node_t &node = nodes[i];
		...
		*buf++ = ' '; size--;
		...
		len = tag.funcname->View().length();
		memcpy( buf, tag.funcname->View().data(), sq_rsl(len));
		buf += len; size -= len;

		*buf++ = ','; size--;
		*buf++ = ' '; size--;

		len = tag.funcsrc->View().length();
		memcpy( buf, tag.funcsrc->View().data(), sq_rsl(len));
		buf += len; size -= len;

		if ( tag.funcname->View().front() != '0' )
		{
			*buf++ = ' '; size--;
			*buf++ = '('; size--;
	...
	static void PrintTime( sample_t us, char *&buf, int &size )
	{
...
typedef enum
{
	_VARREF_TYPE_SCOPE_FIRST = 0,
		VARREF_SCOPE_LOCALS = _VARREF_TYPE_SCOPE_FIRST,
		VARREF_SCOPE_OUTERS,
		VARREF_STACK,
	_VARREF_TYPE_SCOPE_LAST = VARREF_STACK,
	_VARREF_TYPE_OBJECT_FIRST,
		VARREF_OBJ = _VARREF_TYPE_OBJECT_FIRST,
		VARREF_INSTRUCTIONS,
		VARREF_OUTERS,
		VARREF_LITERALS,
		...
		VARREF_ATTRIBUTES,
		VARREF_CALLSTACK,
	_VARREF_TYPE_OBJECT_LAST = VARREF_CALLSTACK,
	VARREF_BREAKPOINT,
	VARREF_MAX
} EVARREF;
...
inline bool IsScopeRef( EVARREF type )
{
	...
	return ( type >= _VARREF_TYPE_SCOPE_FIRST && type <= _VARREF_TYPE_SCOPE_LAST );
}

inline bool IsObjectRef( EVARREF type )
{
	Assert( type >= 0 && type < VARREF_MAX );
	return ( type >= _VARREF_TYPE_OBJECT_FIRST && type <= _VARREF_TYPE_OBJECT_LAST );
}
...
#define Print(...) \
	{ \
		...
		if (m_Print) m_Print( m_pCurVM, __VA_ARGS__ ); \
	}

#define PrintError(...) \
	{ \
		_OutputDebugStringFmt( __VA_ARGS__ ); \
		if (m_PrintError) m_PrintError( m_pCurVM, __VA_ARGS__ ); \
	}
...
#endif
	void OnRequest_SetVariable_Breakpoint( /*const varref_t *ref,*/ string_t &strName, string_t &strValue, int seq );
...
public:
	void OnScriptCompile(std::optional<std::string_view> script,
...
private:
	int ToVarRef( EVARREF type/*, int data*/ );
...
private:
	...
	sqstring_t PrintDisassembly( SQClosure *target, char *scratch, int bufsize );
...
private:
	...
	void DebugHook( HSQUIRRELVM vm, int type,
			char const* sourcename, int line, char const* funcname );
#ifndef SQDBG_DISABLE_PROFILER_AUTO
...
public:
	...
	static const SQVM::CallInfo *GetCurrentScriptSource( HSQUIRRELVM vm );
	static void SQPrint( HSQUIRRELVM vm, std::string_view sv);
	static void SQError( HSQUIRRELVM vm, std::string_view sv);

	void SQPrintAtFrameV( SQPRINTFUNCTION fn,
			HSQUIRRELVM vm, const SQVM::CallInfo *ci, std::string_view sv );

#ifndef SQDBG_CALL_DEFAULT_ERROR_HANDLER
	void SQErrorNoFrame( HSQUIRRELVM vm, std::string_view sv);
	void PrintVar( HSQUIRRELVM vm, const char *name, const SQObjectPtr &obj );
...
#else
	return SQString::Create( _ss(dbg->m_pRootVM), str.ptr );
#endif
}
...
inline bool SQTable_Get( SQDebugServer *dbg, SQTable *table, const string_t &key, SQObjectPtr &val )
{
	...
	#else
		SQObjectPtr str = SQString::Create( _ss(dbg->m_pRootVM), tmp, len );
		return table->Get( str, val );
	#endif
#else
	(void)dbg;
	...
	#else
		return SQTable_Get( SS_NOGC(_ss(dbg->m_pRootVM),) table, key, val );
	#endif
#endif
}
...
void SQDebugServer::Attach( HSQUIRRELVM vm )
{
...

	{
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_define_class") );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_disassemble") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_break") );
		sq_pushobject( m_pRootVM, ref );
		sq_newclosure( m_pRootVM, &SQDebugServer::SQBreak, 1 );
		sq_setnativeclosurename( m_pRootVM, -1, _SC("sqdbg_break") );
		sq_setparamscheck(m_pRootVM, 1, {});
		sq_newslot( m_pRootVM, -3, SQFalse );

#ifndef SQDBG_DISABLE_COMPILER
		sq_pushstring( m_pRootVM, _SC("sqdbg_watch") );
		sq_pushobject( m_pRootVM, ref );
...
#ifndef SQDBG_DISABLE_EVAL_FUNC
		sq_pushstring( m_pRootVM, _SC("sqdbg_eval") );
		sq_pushobject( m_pRootVM, ref );
...
#ifndef SQDBG_DISABLE_PROFILER
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_start") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_stop") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_pause") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_resume") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_reset") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_begin") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_end") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_gets") );
		sq_pushobject( m_pRootVM, ref );
		...
		sq_pushstring( m_pRootVM, _SC("sqdbg_prof_print") );
		sq_pushobject( m_pRootVM, ref );
	...
	Print(fmt::format(_SC("(sqdbg) [{}] Attached\n"), SQDBG_SV_VER));
}
...
bool SQDebugServer::ListenSocket( unsigned short port )
{
	...
	if ( m_Server.IsListening() )
	{
		...
		if ( port )
		{
			Print(fmt::format(_SC("(sqdbg) Socket already open on port {}\n"), port));
		}
		else
		{
	...
	Print(fmt::format(_SC("(sqdbg) Listening for connections on port {}\n"), port));
	return true;
}
...
{
	Print(fmt::format(_SC("(sqdbg) Client connected from {}\n"), addr));
...
#define GET_OR_FAIL( _base, _val ) \
	...
	{ \
		PrintError( fmt::format(_SC("(sqdbg) Invalid DAP message, could not find '{}'\n"), #_val )); \
		DisconnectClient(); \
		return; \
	}
...
void SQDebugServer::OnMessageReceived( char *ptr, int len )
{
	...
	if ( parser.GetError() )
	{
		PrintError(fmt::format(_SC("(sqdbg) Invalid JSON : {}\n"), parser.GetError()));
	...
	else if ( type.IsEqualTo( "response" ) )
	{
		...
		PrintError(fmt::format(_SC("(sqdbg) Unrecognised response '{}'\n"), command.ptr));
	...
	else if ( type.IsEqualTo( "event" ) )
	{
		...
		PrintError(fmt::format(_SC("(sqdbg) Unrecognised event '{}'\n"), event.ptr));
		AssertClientMsg1( 0, "Unrecognised event '%s'", event.ptr );
	}
	else
	{
		PrintError(std::format(_SC("(sqdbg) Invalid DAP type '{}'\n"), type.ptr));
	...
	else if ( command.IsEqualTo( "source" ) )
	{
		...
		if ( arguments->GetTable( "source", &source ) )
		{
			...
			if ( scr )
			{
				...
				DAP_SET_TABLE( body );
					body.SetString( "content", string_t{ scr->scriptptr, scr->scriptlen } );
				DAP_SEND();
				return;
			}
		}
...
void SQDebugServer::OnScriptCompile(std::optional<std::string_view> script,
		std::optional<std::string_view> sourcename )
{
	...
	unsigned int scriptbufsize = ALIGN(scstombslen(script), 64);
	...
	if ( scr->scriptptr )
	{
		scr->scriptlen = scstombs( scr->scriptptr, scriptbufsize, script );
	}
}
...
void SQDebugServer::OnRequest_Initialize( const json_table_t &arguments, int seq )
{
	...
	{
		Print(fmt::format(_SC("(sqdbg) Client initialised: {}\n"),
				clientName.ptr));
	}
	else
	{
...
int SQDebugServer::CompareObj( const SQObjectPtr &lhs, const SQObjectPtr &rhs )
{
	...
	if ( tl == tr )
	{
		...
		switch ( tl )
		{
			...
			case OT_STRING:
			{
				int res = scstrcmp( _string(lhs)->View().data(), _string(rhs)->View().data());
...
bool SQDebugServer::CheckDataBreakpoints( HSQUIRRELVM vm )
{
	...
	for ( unsigned int i = m_DataWatches.Size(); i--; )
	{
		...
		if ( Get( dw.obj, value ) )
		{
			...
			SQPrint( vm, fmt::format(_SC("(sqdbg) Data breakpoint hit: {}\n"), buf.ptr) );
...
static inline bool HasEscapes( const char *src, SQInteger len )
{
		...
		switch ( *src )
		{
			...
			default:
...
#endif
				{
					...
					if ( ret > 0 )
#else
					int ret = IsValidUTF8( src, end - src );
					if ( ret != 0 )
#endif
					{
						src += ret - 1;
	...
	for ( ; dst < strEnd; dst++ )
	{
		switch ( *dst )
		{
			...
			default:
				...
				{
					int ret = IsValidUTF8( dst, strEnd - dst );
					if ( ret == 0 )
					{
#ifdef SQUNICODE
						const int charcount = sizeof(uint16_t) * 2;
						uint16_t val = (uint16_t)*(unsigned char*)dst;
#else
						const int charcount = sizeof(char) * 2;
						unsigned char val = *(unsigned char*)dst;
#endif

						if ( dst + 1 + charcount >= memEnd )
						{
							do
							{
								*dst++ = '?';
						...
						_memmove( dst + 2 + charcount,
							dst + 1,
							memEnd - ( dst + 2 + charcount ),
							memEnd );
...
shift_one:
			...
			case 'x':
			{
				atox( { dst + 2, sizeof(char) * 2 }, (char*)dst );
				_shift( 1, 2 + sizeof(char) * 2 );
				break;
			}
		}
...
static inline int printhex( int flags, C *buf, int size, I value )
{
	int len;
	...
	if ( !( flags & kFS_Uppercase ) )
	{
		if ( !( flags & kFS_NoPrefix ) )
		{
			...
			{
				len = printhex< false, true, false >( buf, size, value );
			}
			else
			{
				len = printhex< true, true, false >( buf, size, value );
			}
		}
		else
		{
			if ( !( flags & kFS_Padding ) )
			{
				len = printhex< false, false, false >( buf, size, value );
			}
			else
			{
				len = printhex< true, false, false >( buf, size, value );
			}
		}
	}
	else
	{
		if ( !( flags & kFS_NoPrefix ) )
		{
			if ( !( flags & kFS_Padding ) )
			{
				len = printhex< false, true, true >( buf, size, value );
			}
			else
			{
				len = printhex< true, true, true >( buf, size, value );
			}
		}
		else
		{
			if ( !( flags & kFS_Padding ) )
			{
				len = printhex< false, false, true >( buf, size, value );
			}
			else
			{
				len = printhex< true, false, true >( buf, size, value );
			}
		}
	}
...
template < typename C, typename I >
static inline int printbin( int flags, C *buf, int size, I value )
{
	int len;
	...
	if ( !( flags & kFS_NoPrefix ) )
	{
		if ( !( flags & kFS_Padding ) )
		{
			len = printbin< false, true >( buf, size, value );
		}
		else
		{
			len = printbin< true, true >( buf, size, value );
		}
	}
	else
	{
		if ( !( flags & kFS_Padding ) )
		{
			len = printbin< false, false >( buf, size, value );
		}
		else
		{
			len = printbin< true, false >( buf, size, value );
		}
	}
	...
	switch ( sq_type(obj) )
	{
...
#else
				int escapes = CountEscapes( _string(obj)->View().data(), _string(obj)->View().length());
				unsigned int size = 2 + _string(obj)->View().length() + escapes;
#endif
...
#else
				len = scstombs( buf + 1, size - 2, _string(obj)->View());
		...
		case OT_FLOAT:
		{
			if ( flags & kFS_Decimal )
			{
				...
				{
					const int size = 2 + sizeof(SQFloat) * 2;
					char *buf = ScratchPad( size );
					int len = printhex( flags, buf, size, *(SQFloat_Int*)&_float(obj) );
					return { buf, (unsigned int)len };
				}
			}
			else if ( flags & kFS_Binary )
			{
				const int size = 2 + sizeof(SQFloat) * 8;
				char *buf = ScratchPad( size );
				int len = printbin( flags | kFS_Padding, buf, size, *(SQFloat_Int*)&_float(obj) );
				return { buf, (unsigned int)len };
			}
			else
			{
getfloat:
				...
				if ( !val.ptr || ( flags & ( kFS_Float | kFS_FloatE | kFS_FloatG ) ) )
				{
					...
					{
						val.len = snprintf( val.ptr, size, "%." FMT_FLT_DIG_STR "g", _float(obj) );
					}
					else
					{
						val.len = snprintf( val.ptr, size, "%." FMT_FLT_DIG_STR "f", _float(obj) );
					}
		...
		case OT_INTEGER:
		{
			if ( flags & kFS_Character )
			{
				...
				if ( _integer(obj) >
						( (SQInteger)(SQUnsignedChar)-1 >> (int)( sizeof(char) >= sizeof(SQInteger) ) ) ||
					_integer(obj) <
						-( ( (SQInteger)(SQUnsignedChar)-1 >> 1 ) + 1 ) )
				{
					if ( flags & kFS_Hexadecimal )
					{
						len = printhex< false >( buf, size, (SQUnsignedInteger)_integer(obj) );
					}
					else if ( flags & kFS_Octal )
					{
						len = printoct( buf, size, (SQUnsignedInteger)_integer(obj) );
					}
					else if ( flags & kFS_Binary )
					{
						len = printbin< false >( buf, size, (SQUnsignedInteger)_integer(obj) );
					}
					else
					{
				...
				{
					len = printoct( buf, size, (SQUnsignedInteger)ch );
				}
				else if ( flags & kFS_Binary )
				{
					len = printbin< false >( buf, size, (SQUnsignedInteger)ch );
				}
				else
				{
...
#ifdef SQUNICODE
				else if ( IsValidUnicode( (SQChar*)&ch, 1 ) > 0 )
				{
				...
				else
				{
					...
					{
						buf[len++] = 'u';
						len += printhex< true, false, false >( buf + len, size, (uint16_t)ch );
					}
#else
					buf[len++] = 'x';
					len += printhex< true, false, false >( buf + len, size, (unsigned char)ch );
			...
			{
				const int size = 2 + sizeof(SQInteger) * 8;
				char *buf = ScratchPad( size );
				int len = printbin( flags, buf, size, (SQUnsignedInteger)_integer(obj) );
				return { buf, (unsigned int)len };
			}
			else if ( flags & kFS_Octal )
			{
			...
			else if ( ( flags & kFS_Hexadecimal ) && !( flags & kFS_Decimal ) )
			{
				...
				char *buf = ScratchPad( size );
				int len = printhex( flags, buf, size, (SQUnsignedInteger)_integer(obj) );
				return { buf, (unsigned int)len };
			}
			else
			{
		...
		case OT_INSTANCE:
		{
			...
			if ( def )
			{
				...
				if ( RunClosure( *def, &obj, res ) && sq_type(res) == OT_STRING )
				{
					...
					{
						unsigned int size = scstombslen( _string(res)->View() );
						char *buf = ScratchPad( size );

						if ( !buf )
							return { STR_NOMEM, STRLEN(STR_NOMEM) };

						unsigned int len = scstombs( buf, size, _string(res)->View());
						return { buf, len };
					}
				}
			}
...
#endif
		{
			...
			{
				int size = FMT_PTR_LEN + 1 + scstombslen( _string(*name)->View());
				char *buf = ScratchPad( size );
				...
				buf[len++] = ' ';
				len += scstombs( buf + len, size - FMT_PTR_LEN - 1, _string(*name)->View());
				return { buf, (unsigned int)len };
			}
		...
		case OT_USERDATA:
		{
			const int size = FMT_PTR_LEN;
			char *buf = ScratchPad( size );
			int len = printhex( buf, size, (uintptr_t)_userdataval(obj) );
			return { buf, (unsigned int)len };
		}
		default:
		default_label:
...
bool SQDebugServer::CompileScript( const string_t &script, SQObjectPtr &out )
{
	...
	*buf = 0;
	...
	if ( SQ_SUCCEEDED( sq_compilebuffer( m_pCurVM, scratch, _SC("sqdbg"), SQFalse ) ) )
	{
		...
		const SQObject *env, SQObjectPtr &out, bool multiline )
{
	...
	*buf = 0;
	...
	if ( SQ_SUCCEEDED( sq_compilebuffer( vm, scratch, _SC("sqdbg"), SQFalse ) ) )
	{
		// Don't create varargs on calls
...
SQInteger SQDebugServer::SQMM_Get( HSQUIRRELVM vm )
{
	...
	SQObjectPtr frame;
	Verify( SQTable_Get( SS_NOGC(_ss(vm),) _table(mtenv)->_delegate, _SC(KW_CALLFRAME), frame ) );
		...
		const SQObject &mtenv, const SQObject &index, SQObjectPtr &value )
{
	...
	if ( ci && sq_type(ci->_closure) == OT_CLOSURE && sq_type(index) == OT_STRING )
	{
		...
		if ( _string(index)->View().length() == 6 || _string(index)->View().length() == 7)
		{
	...
	// env
	SQObjectPtr env;
	Verify( SQTable_Get( SS_NOGC(_ss(vm),) _table(mtenv)->_delegate, _SC(KW_DELEGATE), env ) );
	...
	if ( !SQTable_Get( SS_NOGC(_ss(vm),) _table(mtenv)->_delegate, _SC(KW_ROOT), root ) ||
			sq_type(root) != OT_TABLE )
	{
...
binary:
				...
				case '?':
				{
					...
					if ( !IsFalse( val ) )
					{
						...
						if ( closer != Token_End || m_prevToken == ';' )
							m_cur--;
					}
					else
					{
						...
						switch ( token.type )
						{
							...
							case ';':
							{
								if ( closer == Token_End )
								{
									...
									return Evaluate( dbg, vm, frame, val, closer );
								}
		...
		switch ( op )
		{
			...
			case Token_NotIn:
			{
				switch ( sq_type(rhs) )
				{
					...
					case OT_STRING:
						SetBool( out, sq_type(lhs) == OT_INTEGER &&
								_integer(lhs) >= 0 && _integer(lhs) < _string(rhs)->View().length() );
						break;
					default:
						SetBool( out, false );
				}
			...
			case Token_Pow:
			{
				...
				if ( ( tl | tr ) == OT_INTEGER )
				{
					out = (SQFloat)pow( (double)_integer(lhs), (double)_integer(rhs) );
					return true;
				}
				else if ( ( tl | tr ) == OT_FLOAT )
				{
					SQFloat fl = _float(lhs);
					SQFloat fr = _float(rhs);

					out = (SQFloat)pow( fl, fr );
					return true;
				}
				else if ( tl == OT_INTEGER && tr == OT_FLOAT )
				{
					SQFloat fl = (SQFloat)_integer(lhs);
					SQFloat fr = _float(rhs);

					out = (SQFloat)pow( fl, fr );
					return true;
				}
				else if ( tl == OT_FLOAT && tr == OT_INTEGER )
				{
					SQFloat fl = _float(lhs);
					SQFloat fr = (SQFloat)_integer(rhs);
	...
	static bool UnaryOp( SQDebugServer *dbg, int op, SQObjectPtr &val )
	{
		...
		switch ( op )
		{
			...
			case Token_Typeof:
			{
				...
				extern std::string_view GetTypeName( const SQObjectPtr & );
				std::string_view tname = GetTypeName(val);

				if ( !tname.empty() )
				{
					val = SQString::Create( _ss(dbg->m_pRootVM), tname );
					return true;
				}
		...
		if ( c == '\\' )
		{
			...
			switch ( *m_cur )
			{
				...
				case 'x':
				{
					...
					if ( m_cur == pStart ||
							(int)( m_cur - pStart ) > (int)sizeof(char) * 2 ||
							m_cur >= m_end ||
							*m_cur != '\'' )
					{
						token.type = Err_InvalidXEscape;
...
bool SQDebugServer::ArithOp( char op, const SQObjectPtr &lhs, const SQObjectPtr &rhs, SQObjectPtr &out )
{
	...
	{
		int len = _string(lhs)->View().length() * _integer(rhs);
		...
		{
			_OutputDebugStringFmt( fmt::format(_SC("**(sqdbg) Truncated string multiplication (%u)\n" ),
					sq_rsl( _string(lhs)->View().length() * _integer(rhs) )) );
			len = INT_MAX / sizeof(char);
		}

		CScratch_Restore_Auto _sr( &m_Scratch );
		char *tmp = (char*)ScratchPad( sq_rsl(len) );
		...
		for ( int i = 0; i < len; i += (int)_string(lhs)->View().length() )
		{
			memcpy( tmp + i, _string(lhs)->View().data(), sq_rsl(_string(lhs)->View().length()));
		}

		out = SQString::Create( _ss(m_pRootVM), tmp );
		return true;
	}
...
bool SQDebugServer::GetObj_Var( const SQObjectPtr &var, const SQObjectPtr &key, objref_t &out, SQObjectPtr &value )
{
	...
	switch ( sq_type(var) )
	{
		...
		{
			if ( sq_type(key) == OT_INTEGER &&
					_integer(key) >= 0 && _integer(key) < _string(var)->View().length() )
			{
				out.type = (objref_t::EOBJREF)( objref_t::INT | objref_t::READONLY );
				// Sign will be extended
				out.val = (int)_string(var)->View().data()[ _integer(key) ];
...
bool SQDebugServer::GetObj_VarRef( const varref_t *ref, string_t &expression, objref_t &out, SQObjectPtr &value )
{
	switch ( ref->type )
	{
		...
		case VARREF_BREAKPOINT:
		{
			return false;
		}
		default:
		{
			PrintError(fmt::format(_SC("(sqdbg) Invalid varref requested ({})\n"), ref->type));
...
void SQDebugServer::OnRequest_Evaluate( const json_table_t &arguments, int seq )
{
	...
	if ( IS_INTERNAL_TAG( expression.ptr ) )
	{
		...
		else if ( expression.IsEqualTo( INTERNAL_TAG("breakpoints") ) )
		{
			DAP_START_RESPONSE( seq, "evaluate" );
			DAP_SET_TABLE( body );
				body.SetIntBrackets( "result", m_Breakpoints.Size() + m_DataWatches.Size() );
				body.SetInt( "variablesReference", ToVarRef( VARREF_BREAKPOINT ) );
				wjson_table_t hint = body.SetTable( "presentationHint" );
				wjson_array_t attributes = hint.SetArray( "attributes" );
				attributes.Append( "readOnly" );
			DAP_SEND();
...
static int _sortkeys( const SQObjectPtr *a, const SQObjectPtr *b )
{
	if ( sq_type(*a) == OT_STRING )
	{
		...
		{
			return scstricmp( _string(*a)->View().data(), _string(*b)->View().data());
		}
		else
		{
			return 1;
...
#define _checkNonStringMembers(key) \
	( sq_type(key) != OT_STRING || \
	  _string(key)->View().length() == 0 || \
	  HasEscapes( _string(key)->View().data(), _string(key)->View().length() ) )
			...
			DAP_SET_TABLE( body );
				...
				switch ( sq_type(target) )
				{
					...
					case OT_USERDATA:
					{
						{
							wjson_table_t elem = variables.AppendTable();
							elem.SetString( "name", "size" );
							elem.SetString( "value", GetValue( _userdata(target)->_size ) );
							elem.SetInt( "variablesReference", -1 );
							SetVirtualHint( elem );
						}
						{
							wjson_table_t elem = variables.AppendTable();
							elem.SetString( "name", "tag" );
							{
								jstringbuf_t buf = elem.SetStringAsBuf( "value" );
								buf.PutHex( (uintptr_t)_userdata(target)->_typetag );
							}
							elem.SetInt( "variablesReference", -1 );
							SetVirtualHint( elem );
						}
...
obj_done:;
		...
		case VARREF_STACK:
		{
			...
			DAP_SET_TABLE( body );
				wjson_array_t variables = body.SetArray( "variables" );
				DumpStack( vm, frame, flags, variables );
			DAP_SEND();
...
void SQDebugServer::DumpStack( HSQUIRRELVM vm, int frame, int flags, wjson_array_t &variables )
{
	int stackbase = 0;
	int callframe = -1;
...
void SQDebugServer::DumpBreakpoints( wjson_array_t &variables )
{
	// line breakpoint
	for ( unsigned int i = 0; i < m_nFunctionBreakpointsIdx; i++ )
	{
		const breakpoint_t &bp = m_Breakpoints[i];

		wjson_table_t elem = variables.AppendTable();
		elem.SetIntBrackets( "name", bp.id );
		{
			jstringbuf_t value = elem.SetStringAsBuf( "value" );
			value.Puts( bp.src );

			if ( bp.line )
			{
				value.Put(':');
				value.PutInt( bp.line );
			}
		}
		elem.SetInt( "variablesReference", -1 );
	}
	...
	for ( unsigned int i = 0; i < m_DataWatches.Size(); i++ )
	{
		...
		wjson_table_t elem = variables.AppendTable();
		elem.SetIntBrackets( "name", dw.id );
		{
			jstringbuf_t value = elem.SetStringAsBuf( "value" );
			...
			{
				value.Put('[');
				value.PutHex( (uintptr_t)_refcounted(dw.container->_obj) );
				value.Put(' ');
				value.Puts( GetType( dw.container->_obj ) );
				value.Put(']');
				value.Put('-');
				value.Put('>');
				value.Puts( dw.name );
...
void SQDebugServer::OnRequest_SetVariable( const json_table_t &arguments, int seq )
{
	...
	if ( ref->type == VARREF_INSTRUCTIONS )
	{
#ifdef SQDBG_SUPPORTS_SET_INSTRUCTION
	...
	else if ( ref->type == VARREF_BREAKPOINT )
	{
		OnRequest_SetVariable_Breakpoint( strName, strValue, seq );
		return;
	}
	...
	if ( !GetObj_VarRef( ref, strName, obj, dummy ) )
	{
		...
		DAP_ERROR_BODY( 0, "identifier '{name}' not found" );
			...
			if ( ref->type == VARREF_OBJ && ref->obj.hasNonStringMembers && strName.ptr[-1] == '\"' )
			{
				...
				Escape( strName.ptr, &strName.len, strName.len * ( sizeof(char) * 2 + 2 ) );
			}
...
void SQDebugServer::OnRequest_SetVariable_Breakpoint(
		string_t &strName,
		string_t &strValue,
		int seq )
{
	...
	{
		DAP_ERROR_RESPONSE( seq, "setVariable" );
		DAP_ERROR_BODY( 0, "invalid index" );
			error.SetBool( "showUser", true );
		DAP_SEND();
		return;
	}
	...
	{
		DAP_ERROR_RESPONSE( seq, "setVariable" );
		DAP_ERROR_BODY( 0, "editing breakpoints is not supported, empty value to remove" );
			error.SetBool( "showUser", true );
		DAP_SEND();
		return;
	}

	AssertClient( strName.ptr[0] == '[' && strName.ptr[ strName.len - 1 ] == ']' );

	int breakpointId;
	strName.ptr++;
	strName.len -= 2;
	atoi( strName, &breakpointId );
	...
	if ( breakpointId > 0 && breakpointId < m_nBreakpointIndex )
	{
		for ( unsigned int i = 0; i < m_Breakpoints.Size(); i++ )
		{
			...
			if ( bp.id == breakpointId )
			{
				...
				if ( i < m_nFunctionBreakpointsIdx )
...
int SQDebugServer::ToVarRef( EVARREF type )
{
	Assert( type >= 0 && type < VARREF_MAX );
	Assert( !IsObjectRef( type ) && !IsScopeRef( type ) );

	for ( unsigned int i = 0; i < m_Vars.Size(); i++ )
	{
		const varref_t &v = m_Vars[i];
		if ( v.type == type )
		{
			return v.id;
		}
...
int SQDebugServer::ToVarRef( EVARREF type, const SQObject &obj, bool isStrong )
{
	...
	{
		varref_t &v = m_Vars[i];
		if ( v.type == type && _userpointer(v.GetVar()) == _userpointer(obj) )
		{
			ConvertToWeakRef( v );
			return v.id;
		}
	...
	Assert( _userpointer(var->obj.weakref->_obj) == _userpointer(obj) );
...
void SQDebugServer::DefineClass( SQClass *target, SQTable *params )
{
	...
	if ( SQTable_Get( SS_NOGC(_ss(m_pRootVM),) params, _SC("name"), name ) )
	{
		if ( sq_type(name) == OT_STRING && _string(name)->View().length() )
		{
			CScratch_Restore_Auto _sr( &m_Scratch );
	...
	if ( SQTable_Get( SS_NOGC(_ss(m_pRootVM),) params, _SC("value"), value ) )
	{
	...
	if ( SQTable_Get( SS_NOGC(_ss(m_pRootVM),) params, _SC("metamembers"), metamembers ) )
	{
	...
	if ( SQTable_Get( SS_NOGC(_ss(m_pRootVM),) params, _SC("custommembers"), custommembers ) )
	{
...
sqstring_t SQDebugServer::PrintDisassembly( SQClosure *target, char *scratch, int bufsize )
{
	SQFunctionProto *func = _fp(target->_function);
	char *buf = scratch;
...
#undef putint
	...
	*buf++ = '\n';
	...
	for ( int i = 0; i < nparams; i++ )
	{
		...
		int len = min( (int)_string(param)->View().length(), DISASM_MAX_PARAM_NAME_LEN);
		memcpy( buf, _string(param)->View().data(), sq_rsl(len));
		buf += len;
...
#else
		int len = scsprintf( buf, _bs / (int)sizeof(char), _SC("%-6d %-29s"), index++, tmp.ptr );

		if ( len < 0 || len > _bs / (int)sizeof(char) )
			len = _bs / (int)sizeof(char);
...
sqstring_t SQDebugServer::ProfGets( HSQUIRRELVM vm, SQString *tag, int type )
{
	...
	char *buf = (char*)ScratchPad( sq_rsl(size) );
...
void SQDebugServer::ProfPrint( HSQUIRRELVM vm, SQString *tag, int type )
{
	...
	// Print each line
	for ( char *start = str.ptr; ; )
	{
		char *end = scstrchr( start, '\n' );
		if ( !end )
			break;

		int linelen = (int)( end + 1 - start );

		_OutputDebugStringFmt( fmt::format(_SC(FMT_VSTR), linelen, start) );
		m_Print( vm, fmt::format(_SC(FMT_VSTR), linelen, start ));
...
#ifndef SQDBG_CALL_DEFAULT_ERROR_HANDLER
void SQDebugServer::PrintVar( HSQUIRRELVM vm, const char *name, const SQObjectPtr &obj )
{
	switch ( sq_type(obj) )
	{
		case OT_NULL:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] NULL\n"), name) );
			break;
		case OT_INTEGER:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] {}\n"), name, _integer(obj)) );
			break;
		case OT_FLOAT:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] %.{}g\n"), name, _float(obj)) );
			break;
		case OT_USERPOINTER:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] USERPOINTER {}\n"), name, (uintptr_t)_userpointer(obj)) );
			break;
		case OT_STRING:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] \"{:.50}\"\n"), name, _string(obj)->View().data()) );
			break;
		case OT_TABLE:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] TABLE (#{})\n"), name, _table(obj)->CountUsed()) );
			break;
		case OT_ARRAY:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] ARRAY (#{})\n"), name, _array(obj)->Size()) );
			break;
		case OT_CLOSURE:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] CLOSURE\n"), name) );
			break;
		case OT_NATIVECLOSURE:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] NATIVECLOSURE\n"), name) );
			break;
		case OT_GENERATOR:
		{
			...
			{
				SQErrorNoFrame( vm, fmt::format(_SC("[{}] GENERATOR ({})\n"), name, _string(funcname)->View().data()) );
			}
			else
			{
				SQErrorNoFrame( vm, fmt::format(_SC("[{}] GENERATOR\n"), name) );
		...
		case OT_USERDATA:
			SQErrorNoFrame( vm, fmt::format(_SC("[{}] USERDATA {}\n"), name, (uintptr_t)_userdataval(obj)) );
			break;
		case OT_THREAD:
			SQErrorNoFrame( vm,fmt::format( _SC("[{}] THREAD\n"), name) );
			break;
		case OT_CLASS:
		{
			const classdef_t *def = FindClassDef( _class(obj) );
			...
			if ( def && def->name.ptr )
			{
				SQErrorNoFrame( vm, fmt::format(_SC("[{}] CLASS ({})\n"),
		...
		case OT_INSTANCE:
		{
			...
			if ( def && def->name.ptr )
			{
				SQErrorNoFrame( vm, fmt::format(_SC("[{}] INSTANCE ({})\n"),
						name, def->name.ptr + FMT_PTR_LEN + 1) );
			}
			else
			{
...
void SQDebugServer::PrintStack( HSQUIRRELVM vm )
{
	...
	while ( frame-- )
	{
		...
		if ( sq_type(ci._closure) == OT_CLOSURE )
		{
			...
			if ( sq_type(func->_name) == OT_STRING )
				fn = _string(func->_name)->View().data();

			if ( sq_type(func->_sourcename) == OT_STRING )
				src = _string(func->_sourcename)->View().data();
		}
		else if ( sq_type(ci._closure) == OT_NATIVECLOSURE )
		{
			SQNativeClosure *closure = _nativeclosure(ci._closure);
			...
			if ( sq_type(closure->_name) == OT_STRING )
				fn = _string(closure->_name)->View().data();
		}
		else UNREACHABLE();

		SQErrorNoFrame( vm, fmt::format(_SC("*FUNCTION [{}()] {} line [{}]\n"), fn, src, line) );
	}
	...
	while ( frame-- )
	{
		...
		for ( int i = 0; i < func->_nlocalvarinfos; i++ )
		{
			...
			if ( var._start_op <= ip && var._end_op + 1 >= ip )
			{
				PrintVar( vm, _string(var._name)->View().data(), vm->_stack._vals[stackbase + var._pos]);
			}
		}
		...
		{
			const SQOuterVar &var = func->_outervalues[i];
			PrintVar( vm, _string(var._name)->View().data(), *_outervalptr(pClosure->_outervalues[i]));
		}
	}
}
#endif
...
#ifdef SQDBG_CALL_DEFAULT_ERROR_HANDLER
	SQObjectPtr dummy;
	vm->Call( m_ErrorHandler, 2, vm->_top-2, dummy, SQFalse, SQFalse );
#else
...
exit:;
			...
			case '$':
			{
				...
				if ( CHECK_KEYWORD("FUNCTION") )
				{
					...
					if ( sq_type(func->_name) == OT_STRING )
					{
						...
						int remaining = bufsize - ( pWrite - buf );
						int writelen = scstombs( pWrite, remaining, name->View());
						pWrite += writelen;
					}
					else
					{
	...
	*pWrite = 0;
	...
	m_Print( vm, fmt::format(_SC(FMT_CSTR), buf) );
		...
		char const* sourcename, int line, char const* funcname )
{
	...
	switch ( type )
	{
		case SQ_HOOK_LINE:
		{
			...
			unsigned int srclen = SQStringFromSQChar( sourcename )->View().length();
			...
			//fmt::print("sqdbg check bp at {}:{}\n", src.ptr, line);
			if ( bp )
			{
				//fmt::print("sqdbg bp at {}:{}\n", src.ptr, line);
					if ( HasCondition( bp ) )
				{
		...
		case SQ_HOOK_CALL:
		{
			...
			if ( funcname )
			{
				unsigned int funclen = SQStringFromSQChar( funcname )->View().length();
...
SQInteger SQDebugServer::SQPrintDisassembly( HSQUIRRELVM vm )
{
	...
	if ( dbg )
	{
		...
		CScratch_Restore_Auto _sr( &dbg->m_Scratch );
		char *scratch = (char*)dbg->ScratchPad( sq_rsl(buflen) );
		AssertOOM( scratch, sq_rsl(buflen) );

		if ( !scratch )
		{
			sq_pushstring( vm, fmt::format(_SC(STR_NOMEM), STRLEN(STR_NOMEM)) );
			return 1;
		}

		sqstring_t str = dbg->PrintDisassembly( _closure(target), scratch, sq_rsl(buflen) );
		sq_pushstring( vm, str.ptr );
		return 1;
	}
...
SQInteger SQDebugServer::SQAddDataBreakpoint( HSQUIRRELVM vm )
{
	...
	if ( dbg && dbg->IsClientConnected() )
	{
		...
		unsigned int size = 2 + scstombslen( _string(expression)->View());

		if ( sq_type(condition) == OT_STRING )
			size += scstombslen( _string(condition)->View());
...
SQInteger SQDebugServer::SQEval( HSQUIRRELVM vm )
{
	...
	if ( dbg )
	{
		...
		unsigned int size = scstombslen( _string(expression)->View()) + 1;
...
const SQVM::CallInfo *SQDebugServer::GetCurrentScriptSource( HSQUIRRELVM vm )
{
...
void SQDebugServer::SQPrint( HSQUIRRELVM vm, std::string_view sv)
{
	SQDebugServer *dbg = sqdbg_get_debugger( vm );
	Assert( dbg && dbg->IsClientConnected() );
	if ( dbg )
	{
		dbg->SQPrintAtFrameV( dbg->m_Print, vm, dbg->GetCurrentScriptSource( vm ), sv );
	}
}

void SQDebugServer::SQError( HSQUIRRELVM vm, std::string_view sv )
{
	SQDebugServer *dbg = sqdbg_get_debugger( vm );
	Assert( dbg && dbg->IsClientConnected() );
	if ( dbg )
	{
		dbg->SQPrintAtFrameV( dbg->m_PrintError, vm, dbg->GetCurrentScriptSource( vm ), sv );
	}
}

#ifndef SQDBG_CALL_DEFAULT_ERROR_HANDLER
void SQDebugServer::SQErrorNoFrame( HSQUIRRELVM vm, std::string_view sv )
{
	this->SQPrintAtFrameV( this->m_PrintError, vm, nullptr, sv );
}
#endif
...
void SQDebugServer::SQPrintAtFrameV( SQPRINTFUNCTION fn,
		HSQUIRRELVM vm, const SQVM::CallInfo *ci, std::string_view sv )
{
	int len = sv.length();
	...
	_OutputDebugString( sv.data() );
	fn( vm, sv );
	std::string const ssv{ sv };
	this->SendEvent_OutputStdOut(ssv, ci);
}
...
SQInteger SQDebugServer::SQDebugHook( HSQUIRRELVM vm )
{
	...
	if ( dbg )
	{
		if ( dbg->IsClientConnected() )
		{
			...
			const char *src = sq_type(sourcename) == OT_STRING ? _string(sourcename)->View().data() : nullptr;
			const char *fun = sq_type(funcname) == OT_STRING ? _string(funcname)->View().data() : nullptr;

			std::string sfile = "(null)";
			if (src) {
				sfile = src;
				if (std::isupper(src[0]) && src[1] == ':' && src[2] == '\\') {
					const_cast<char*>(src)[0] = std::tolower(src[0]);
				}
			}
			std::string sfun = "(null)";
			if (fun) {
				sfun = fun;
			}
...
#if 0
// For debug purposes
void sqdbg_dump_stack( HSQUIRRELVM vm,
			void (*callback)(
				const char *nameptr, int namelen,
				const char *typeptr, int typelen,
				const char *valueptr, int valuelen ) )
{
	HSQDEBUGSERVER dbg = sqdbg_get_debugger( vm );
	if ( !dbg )
		return;

	CScratch_Restore_Auto _sr( &dbg->m_Scratch );
	CBufTmpCache _bufcache( &dbg->m_SendBuf );
	{
	...
	json_table_t table;
	JSONParser parser( &dbg->m_Scratch, dbg->m_SendBuf.Base(), dbg->m_SendBuf.Size(), &table );
	Assert( !parser.GetError() );

	json_array_t *a;
	Verify( table.GetArray( "", &a ) );
	...
	for ( int i = 0; i < a->Size(); i++ )
	{
		...
		(*callback)( name.ptr, name.len, type.ptr, type.len, value.ptr, value.len );
	}
}
#endif
...
HSQDEBUGSERVER sqdbg_get_debugger( HSQUIRRELVM vm )
{
	...
	SQObjectPtr ref;
	if ( SQTable_Get( SS_NOGC(_ss(vm),) _table(_ss(vm)->_registry), _SC(SQDBG_SV_TAG), ref ) )
	{
		Assert( sq_type(ref) == OT_USERDATA );
		return ((CDebuggerScriptRef*)_userdataval(ref))->dbg;
	}
...
{
	SQTable_Get( SS_NOGC(_ss(vm),) _table(_ss(vm)->_registry), _SC(SQDBG_SV_TAG), ref );
...
HSQDEBUGSERVER sqdbg_attach_debugger( HSQUIRRELVM vm )
{
	...
	sq_pushstring( vm, _SC(SQDBG_SV_TAG) );
	...
	if ( SQ_SUCCEEDED( sq_get( vm, -2 ) ) )
	{
		...
		if ( ref->dbg )
		{
			// Update print funcs
#if SQUIRREL_VERSION_NUMBER >= 300
			ref->dbg->m_Print = sq_getprintfunc( ref->dbg->m_pRootVM );
			ref->dbg->m_PrintError = sq_geterrorfunc( ref->dbg->m_pRootVM );
#else
			ref->dbg->m_Print = sq_getprintfunc( ref->dbg->m_pRootVM );
			ref->dbg->m_PrintError = ref->dbg->m_Print;
#endif
			if ( sq_type(ref->dbg->m_ErrorHandler) != OT_NULL )
				sq_release( ref->dbg->m_pRootVM, &ref->dbg->m_ErrorHandler );

			ref->dbg->m_ErrorHandler = ref->dbg->m_pRootVM->_errorhandler;

			if ( sq_type(ref->dbg->m_ErrorHandler) != OT_NULL )
				sq_addref( ref->dbg->m_pRootVM, &ref->dbg->m_ErrorHandler );

			Assert( ref->dbg->m_Print && ref->dbg->m_PrintError );
...
void sqdbg_on_script_compile( HSQDEBUGSERVER dbg,
		...
		std::optional<std::string_view> sourcename )
{
	dbg->OnScriptCompile( script, sourcename );
}
